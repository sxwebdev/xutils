...
| `pipeline`   | `github.com/sxwebdev/xutils/pipeline`   | Need declarative resumable workflow with branching/repeats |
...
| ----------------- | ----------------------------------- | --------------------------------------- |
| Step types        | Action, Poll, Branch, Repeat        | Single step function                    |
| Structure         | Nested branches and repeats         | Stages → Steps hierarchy                |
| State persistence | Fail-stop RunState JSON (external)  | Snapshot JSON (external)                |
...
| Retry             | RetryConfig + scheduler RetryAfter  | Per-step RetryPolicy (linear/backoff)   |
